...
class WebRTCSignalService(
    ...
    fun handleIdentify(jwt: String, session: WebSocketSession): IdentifiedMessageResponseDto =
        IdentifiedMessageResponseDto(
    ...
    fun handleCallGroup(
        ...
        callGroupMessageDto: CallGroupMessageDto
    ): Pair<NewUserMessageResponseDto, WebSocketSession>? {
    ...
    fun handleResponse(
        session: WebSocketSession,
        makeAnswerMessageDto: MakeAnswerMessageDto
    ): Pair<NewAnswerMessageResponseDto, WebSocketSession>? {
        val userGroup = sessions[session] ?: return null
        try {
    ...
    fun handleConnect(
        session: WebSocketSession,
        connectGroupMessageDto: ConnectGroupMessageDto
    ): ConnectedStatusMessageResponseDto {
        val userGroup = sessions[session] ?: return ConnectedStatusMessageResponseDto(false)
        return ConnectedStatusMessageResponseDto(
            try {
    ...
    fun handlePresent(session: WebSocketSession): PresentMessageResponseDto? {
        ...
        val users =
            sessions.filter { it.value.second?.id == userGroup.second?.id && it.value.first.id != userGroup.first.id }
                .map { it.value.first.id }
        return PresentMessageResponseDto(users)
    }

    fun handleCandidate(
        session: WebSocketSession,
        addCandidateMessageDto: AddCandidateMessageDto
    ): Pair<ICECandidateMessageResponseDto, WebSocketSession>? {
        val userGroup = sessions[session] ?: return null
        userGroup.second ?: return null
